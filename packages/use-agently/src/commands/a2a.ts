...
import { createPaymentFetch, createDryRunFetch, createA2AClient } from "../client.js";
...
export const a2aCommand = new Command("a2a")
  ...
  .action(async (agentUri: string, options: { message: string }, command: Command) => {
    ...
    const { pay } = command.optsWithGlobals<{ pay?: boolean }>();
    const fetchImpl = pay ? createPaymentFetch(wallet) : createDryRunFetch();
